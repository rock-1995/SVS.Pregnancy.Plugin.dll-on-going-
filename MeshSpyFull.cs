    ...
    internal static class MeshSpyFull
    {
        ...
        private static bool TryBuildReadableClothMesh(SkinnedMeshRenderer smr, Mesh original, out Mesh replacement, out string detail)
        {
            ...
            try
            {
                ...
                replacement = mesh;
                detail = $"bakedVerts={verts.Length} tris={tris.Length} bindposes={bindposes.Length} boneWeights={boneWeights.Length} boundsC={Fmt(mesh.bounds.center)} boundsS={Fmt(mesh.bounds.size)}";
                return true;
            }
            catch (Exception e)
            {
